...
struct Problem {
    ...
    Problem(unsigned vertices, const char* family)
        ...
        parameters.add("poisson_stiffness_mode", std::string("constant_preserving"));
...
TEST(FEMConstantPreservation, RawActionIsInvariantUnderConstantShift) {
    for (const auto* family : {"gll", "equispaced"}) {
        ...
        ippl::EvalFunctor<double, 1, Handler::dofsPerElement> eval(
            ippl::Vector<double, 1>(4), 0.25, ippl::PoissonStiffnessMode::ConstantPreserving);
...
TEST(FEMConstantPreservation, FineMeshQuadraticAccuracy) {
    for (const auto* family : {"gll", "equispaced"}) {
        SCOPED_TRACE(family);
