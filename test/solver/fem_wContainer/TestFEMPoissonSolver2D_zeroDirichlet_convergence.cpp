// Convergence study for FEMPoissonSolver_wFEMContainer (Track B), 2D.
//
// Four TASK3 §11.2 sources on [0,1]^2: sine, nonsymmetric, polynomial, affine.
// Lagrange orders P1–P3, Gauss–Legendre quadrature order 9 (all orders).
//
// Writes: convergence_FEMPoissonSolver2D_{gll|equispaced}.dat
//
// Usage:
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence [--info 5]
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --max-nodes 64
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --min-nodes 2 --max-nodes 2
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --solver plain        # unpreconditioned CG
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --solver preconditioned  # Jacobi PCG (default)
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --preconditioner_type jacobi
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --interpolation_nodes gll
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --interpolation_nodes equispaced
//   ./TestFEMPoissonSolver2D_zeroDirichlet_convergence --quadrature_nodes gauss_legendre
...
ConvergenceRow runCase(unsigned num_nodes_per_dim, bool preconditioned, const std::string& precon_type,
    ...
    const unsigned num_cells = num_nodes_per_dim - 1;
    const unsigned nghost    = 1;

    const ippl::Vector<unsigned, Dim> nodes_per_dim(num_nodes_per_dim);
    ...
    params.add("max_iterations", 20000);
    ...
    row.num_nodes             = num_nodes_per_dim;
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        const unsigned max_nodes   = parseMaxNodes(argc, argv, 1u << 6);
        ...
        constexpr const char* domain_note = "[0,1]^2 (homogeneous Dirichlet)";
