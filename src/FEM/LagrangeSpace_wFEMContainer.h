// Class LagrangeSpace_wfc
//    This is the LagrangeSpace_wfc class. It is a class representing a Lagrange space
//    for finite element methods on a structured grid.

#ifndef IPPL_LAGRANGESPACE_WFC_H
#define IPPL_LAGRANGESPACE_WFC_H
...
namespace ippl {
    ...
    public:
         ...
         * @brief Construct a new LagrangeSpace_wfc object
         *
         ...
         */
        LagrangeSpace_wfc(UniformCartesian<T, Dim>& mesh, ElementType& ref_element,
                      const QuadratureType& quadrature, const Layout_t& layout);

        /**
        WORKS WITH DOFHANDLER
         * @brief Construct a new LagrangeSpace_wfc object (without layout)
        ...
        /**
        WORKS WITH DOFHANDLER
         * @brief Initialize a LagrangeSpace_wfc object created with the default constructor
         *
         * @param mesh Reference to the mesh
         * @param layout Reference to the field layout
         */
        ...
        /**
         * @brief Apply the diagonal D = diag(A) to the input coefficients.
         *        ZERO_FACE boundary rows return zero; CONSTANT_FACE rows copy the input.
         *
         * @param field Input coefficients and mesh/layout/BC information.
         * @param evalFunction The bilinear form (same as passed to evaluateAx).
         *
         * @return FieldLHS - D*x on unconstrained DOFs.
         */
        template <typename F>
        FieldLHS evaluateAx_diag(FieldLHS& field, F& evalFunction) const;

        // Strict lower triangle: global row index > global column index (x-fastest).
        template <typename F>
        FieldLHS evaluateAx_lower(FieldLHS& field, F& evalFunction) const;

        // Strict upper triangle: global row index < global column index (x-fastest).
        template <typename F>
        ...
        struct DeviceStruct {
            ...
            // works since numElementDOFs in LagrangeSpace_wfc is static constexpr
            static constexpr unsigned numElementDOFs = LagrangeSpace_wfc::numElementDOFs;
...
#include "FEM/LagrangeSpace_wFEMContainer.hpp"
