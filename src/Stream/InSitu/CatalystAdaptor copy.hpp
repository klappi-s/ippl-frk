...
void CatalystAdaptor::Execute(
    ...
    int cycle, double time, int rank
        ...
        // Use the execute method to get type-safe access
        registry_vis.execute([&](const auto& typed_registry) {
            typed_registry.for_each([&](const std::string& label, const auto& entry) {
                execute_entry(entry, label, node, vr);  // Full type safety preserved!
            });
        });
