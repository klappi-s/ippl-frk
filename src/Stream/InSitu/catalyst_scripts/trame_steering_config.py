...
def preload_steerable_proxies():
    ...
    xml_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "catalyst_proxy.xml")
    ...
    try:
        ...
        # 1. Load into Global Proxy Definition Manager
        try:
        ...
        # 2. Load into Active Session (Builtin)
...
def load_steerable_proxies(proxy_manager=None):
    ...
    if os.path.exists(xml_path):
        try:
            ...
            # If no specific manager provided, try loading into active session
            if servermanager.ActiveConnection:
                 pm = servermanager.ActiveConnection.Session.GetSessionProxyManager()
                 if hasattr(pm, 'LoadConfigurationXMLFromString'):
                     pm.LoadConfigurationXMLFromString(xml_content)
...
def get_steerable_proxy(catalyst_link, proxy_name=None):
    """Find the steerable proxy in the insitu proxy manager."""
    if not catalyst_link: return None
    ...
    # If name provided, try to find it
    if proxy_name:
        # Try direct match
        p = pm.GetProxy("sources", proxy_name)
        if p: return p
        
        # Try iterating to match XML name
        try:
            n = pm.GetNumberOfProxies("sources")
            for i in range(n):
                name = pm.GetProxyName("sources", i)
                p = pm.GetProxy("sources", name)
                if p and p.GetXMLName() == proxy_name: 
                     return p
        except: pass
        return None

    # Fallback for legacy calls (return the first one found or SCALARS)
    try:
        n = pm.GetNumberOfProxies("sources")
        for i in range(n):
            name = pm.GetProxyName("sources", i)
...
def update_steering_parameter(catalyst_link, proxy_name, param_name, value):
    ...
    if proxy:
        ...
        if prop:
            # Normalize incoming values: convert strings/None to proper numeric types
            def to_int(x):
                ...
                try:
                    ...
                    # Fallback: treat truthy/falsey strings
                    sx = str(x).strip()
    ...
    try:
        ...
        for proxy in sources_group.findall("SourceProxy"):
            ...
            for prop in proxy:
                ...
                # Infer num_elements from command if not explicit
                ...
                # Check for domains/widgets
                widget_type = 'text' # default
                domain = {}
                
                # 1. Check direct domain
            ...
            for group in proxy.findall("PropertyGroup"):
                ...
                for prop_ref in group.findall("Property"):
                    ...
                    if p_name in param_map:
                        ...
                        if '.' in p_name:
                            parts = p_name.split('.')
                            # If prefix matches group label, strip it
                            if parts[0] == group_label:
                                p_def['label'] = ".".join(parts[1:])
                            else:
                                p_def['label'] = parts[-1]
                        
                        # Re-check for Prototype Enum/Range/Boolean
                                    ...
                                    if pg is not None:
                                        ...
                                        if proto_proxy is not None:
                                            ...
                                            if proto_prop is not None:
                                                # 0) Boolean domain or checkbox panel
            ...
            # Construct Final Structure for this proxy
            final_structure = []
            for p in raw_params:
