...
public:
     ...
     */
    explicit VisRegistry(std::pair<std::string, Types>... entries)
...
namespace detail {
    ...
    // Helper to convert tuple of types to VisRegistry template
    template<typename Tuple>
    struct tuple_to_registry;
    
    template<typename... Types>
    struct tuple_to_registry<std::tuple<Types...>> {
        using type = VisRegistry<Types...>;
    };
...
/**
 * @brief Factory function to create a VisRegistry with automatic type deduction
 * 
 * Usage: auto registry = MakeVisRegistry("field1", field_obj, "particles", particle_obj);
 * 
 * @param args Alternating label-object pairs
 * @return std::shared_ptr to the created VisRegistry
 */
template<typename... Args>
auto MakeVisRegistry(Args&&... args) {
    static_assert(sizeof...(args) % 2 == 0, "Arguments must come in label-object pairs");
...
auto MakeVisRegistryNamed(Types&&... objects) {
    ...
    using registry_type = VisRegistry<std::decay_t<Types>...>;
